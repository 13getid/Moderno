  ...
  return (
    <div className="min-h-screen bg-stone-100 p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        {stats.map((s) => (
          <div key={s.id} className="bg-white rounded-lg p-5 shadow-sm">
          ...
          <tbody className="divide-y divide-stone-100">
            {recentOrders.map((order) => (
              <tr key={order.id} className="hover:bg-stone-50">
                <td className="px-6 py-4 font-mono text-stone-700">{order.id}</td>
                <td className="px-6 py-4 text-stone-800">{order.customer}</td>
                <td className="px-6 py-4 text-stone-600">{order.product}</td>
